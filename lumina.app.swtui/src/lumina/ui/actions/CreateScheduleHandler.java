 ...
 */
public class CreateScheduleHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent execEvent) throws ExecutionException {
		...
		if (currentSelection instanceof IStructuredSelection) {
			...
			final Object selectedElement = structuredSelection
					.getFirstElement();
			final DeviceTimer enclosingTimer = Queries
					.getAncestorTimer(selectedElement);

			assert enclosingTimer != null;

			final ISelectionProvider selectionProvider = HandlerUtil
					.getActivePart(execEvent).getSite().getSelectionProvider();

			final IUndoableOperation createSchedule = new BaseRedoableOperation(
					"create schedule") {
				private Schedule createdSchedule;

				public IStatus execute(IProgressMonitor monitor, IAdaptable info) {
					final Schedule dropTarget = ModelUtils
							.getDropTargetForSchedule(selectedElement);
					createdSchedule = ProjectModel.getInstance()
							.createSchedule(enclosingTimer, dropTarget);

					SelectionUtils
							.doSelectItems(new Object[] { createdSchedule },
									selectionProvider);
				...
				public IStatus undo(IProgressMonitor monitor, IAdaptable info) {
					if (createdSchedule != null) {
						ProjectModel.getInstance().deleteSchedules(
								new Schedule[] { createdSchedule });
			...
			createSchedule.addContext(IOperationHistory.GLOBAL_UNDO_CONTEXT);
			...
			try {
				undoHistory.execute(createSchedule, null, null);
				...
				if (t instanceof LuminaException)
					...
					Logger.getInstance()
							.log(LogService.LOG_ERROR,
									Messages.getString("CreateScheduleActionHandler.errorCreatingSchedule"), t); //$NON-NLS-1$
			}
